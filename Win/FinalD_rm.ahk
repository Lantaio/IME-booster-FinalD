...
|:: {
	Send "{Blind}{\ Up}{LShift Up}"
	SendText smartChoice('|', '｜')  ; 此符号触发笔画反查功能，但估计此功能不常用，所以直接上屏中纹全角分隔符‘｜’，可再按右Shift键来进行笔画反查。
}
@:: SendText "@"
...
<^^:: Send "{Blind}{LCtrl up}{^}"  ; 在惊喜输入方案中‘^’触发输入扩展符号功能，这里设置当按下Ctrl+Shift+^时发送‘^’给Rime输入法触发此功能
...
>^$:: Send "{Blind}{RCtrl up}{$}"  ; 在惊喜输入方案中‘$’触发输入大写数字和大写金额功能，这里设置当按下Ctrl+Shift+$时发送‘$’给Rime输入法触发此功能
