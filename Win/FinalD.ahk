...
(:: {
	...
	if Tip and commit = '）' and not (BetterCN and WinActive("ahk_group CN"))
		showTip("后", 1)
	if isPair(before, commit) and KeyWait(')', "T" String(Interval))  ; 如果 （在不是自动配对的情况下）前一个标点和本次输入的标点是配对标点，并且是短按，则光标回到配对标点中间
		Send "{Left}"
	; reKeyState "LShift"
}
...
":: {
	...
	if not (BetterCN and WinActive("ahk_group CN")) and shouldEN(before) {
		...
		else if before = '"' and KeyWait('"', "T" String(Interval)) {  ; 如果 （在不是自动配对的情况下）前一个字符和本次输入的标点是配对标点，并且是短按，则光标回到配对标点中间
			Send "{Left}"
		}
	}
	else {  ; 如果应该输入中文
		...
		if commit = '“' {
			...
			if Tip
				showTip("后", 1)
			if before = '“' and KeyWait('"', "T" String(Interval))  ; 如果 （在不是自动配对的情况下）前一个字符和本次输入的标点是配对标点，并且是短按，则光标回到配对标点中间
				Send "{Left}"
		}
	}
	; reKeyState "LShift"  ; 可自动重复
...
{:: {
	...
	if KeyWait(ThisHotkey, "T" String(Interval)) {  ; 短按
		...
		commit := smartChoice('}', '」')
		SendText commit
		if isPair(before, commit) and KeyWait(ThisHotkey, "T" String(Interval))  ; 如果 （在不是自动配对的情况下）前一个标点和本次输入的标点是配对标点，并且是短按，则光标回到配对标点中间
			Send "{Left}"
	}
	else  ; 长按
		Send "{}}"  ; 交给输入法处理
...
':: {
	...
	if not (BetterCN and WinActive("ahk_group CN")) and shouldEN(before) {
		...
		else if before = "'" and KeyWait(ThisHotkey, "T" String(Interval)) {  ; 如果 （在不是自动配对的情况下）前一个字符和本次输入的标点是配对标点，并且是短按，则光标回到成对标点中间
			Send "{Left}"
		}
	}
	else {
		...
		if commit = "‘" {
			...
			if Tip
				showTip("后", 1)
			if before = '‘' and KeyWait(ThisHotkey, "T" String(Interval))  ; 如果 （在不是自动配对的情况下）前一个字符和本次输入的标点是配对标点，并且是短按，则光标回到配对标点中间
				Send "{Left}"
		}
	}
}
