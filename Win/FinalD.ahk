...
smartChoice(en, cn, before?) {
	if not isSet(before)
		before := getBeforeI()
	if AI  ; 智能模式
		; 如果*不是* 当前程序是中文语境软件 并且 前一个内容是西文标点
		if not WinActive("ahk_group CN") and isPrevEN(before)
			Return en
		...
		; 如果*不是* （（前一个内容是换行符 或 空））并且 当前程序是中文语境软件） 并且 前一个内容是西文标点
		if not ((before ~= '`a)\R$' or before = '') and WinActive("ahk_group CN")) and isPrevEN(before)
			Return en
		; 否则，应是中文标点
		else {
			if Tip and cn ~= "，|：|；|？|！|｜|～"
...
":: {
	...
	if KeyWait('"', "T" String(Interval)) {  ; 短按
		before := getBeforeI()  ; （※ 不能放在if语句之前，否则可能会导致 KeyWait检测有问题！）
		; 如果应该输入英文
		if not (AI and WinActive("ahk_group CN")) and isPrevEN(before) {
			SendText '"'
		...
		before := getBeforeI()
		if not (AI and WinActive("ahk_group CN")) and (before = '"' or before = ' ' or before ~= '`a)\R$' or before = '`t' or before = '')
			SendText '"'
		else
			Send ThisHotkey  ; 交给输入法处理
	}
...
':: {
	if KeyWait('"', "T" String(Interval)) {  ; 短按
		...
		before := getBeforeI()
		if not (AI and WinActive("ahk_group CN")) and (before = "'" or before = ' ' or before ~= '`a)\R$' or before = '`t' or before = '')  ; 为了编程时方便连按而设置
			SendText "'"
		else
			Send ThisHotkey  ; 交给输入法处理
	}
...
[:: {  ; ※ 为Markdown优化，英文优先，和“{”键逻辑不同！
	...
	if KeyWait(ThisHotkey, "T" String(Interval)) {  ; 短按
		...
		if not (before = '【' or AI and WinActive("ahk_group CN")) {
			SendText "]"
			if before = '['  ; 如果 （在不是自动配对的情况下）前一个字符和本次输入的标点是配对标点，则光标回到配对标点中间
				Send "{Left}"
		}
