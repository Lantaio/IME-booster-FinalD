...
* 上图第四列“中/英标点变换”的意思是当光标在标点符号后面时，**按右<kbd>Shift</kbd>键可将`→`左边所列的标点快速变换为`→`右边的英文标点符号**。另外，当光标在成对的标点符号中间时，此功能键同样可以自动变换成对的标点符号。右<kbd>Shift</kbd>键还有一个区别于左<kbd>Shift</kbd>键的功能，就是在对`'`和`"`的变换时，只对光标前的标点进行单个变换，而不会对成对的`''`或`""`进行变换，在某些场景下非常有用。此功能可理解为用于快速从中文标点或其它常用标点变换为英文标点。如之后再反复按右<kbd>Shift</kbd>键，则会在`↔`两边的高频中/英标点之间反复变换。所有标点都有此功能，只是为了照顾密集恐惧症人类，只用开头几个标点作为示意。
...
## 扩展功能⚡️
🎊此插件从2.x版开始加入英文字母和数字的漂移功能。当光标在大、小写英文字母后面时，按左<kbd>Shift</kbd>键可以将其变换为相应的大、小写希腊字母。反之，当光标在大、小写希腊字母后面时，按右<kbd>Shift</kbd>键可以将其变换为相应的大、小写英文字母。英文字母和希腊字母的对应关系见下图（黑色为英文字母，红色为大写希腊字母，蓝色为小写希腊字母）：
