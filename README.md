...
## 项目概述⚡️
啥？标点符号也玩漂移？嗯，此中文输入法插件（以下简称此插件）是中/英文标点符号智能输入程序，**以更好记、更快速地输入中/英文标点符号为目标**，突破输入速度的瓶颈，并且免去切换中/英文输入模式，大幅减少因忘记切换模式输错标点而要按<kbd>Backspace</kbd>键的情况。
...
💡️技巧：如果要在表格中输入扩展标点，可以先在表格外输入那个标点符号，然后复制到表格里面。
