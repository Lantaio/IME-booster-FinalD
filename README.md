...
📣️**公告**：2.x版已经推出，此插件已实现全键盘漂移。此说明文档有待更新。
...
但你有没有想过，按<kbd>Backspace</kbd>键，然后切换中/英输入模式，再接着干什么？其实还是按刚才那个标点键啊！也就是说其实我们没有按错键，只是电脑上屏了不是我们期望的标点符号而已。有没有更好的方法，不是我们去做这些既无聊又累人的改正工作，而是由电脑去做？我看行，这便是上图第3列『英/中标点轮换』要做的事情，用左<kbd>Shift</kbd>键来告诉电脑：“你搞错了，给我改过来。” \
以前面的2个情况为例，输入“English”之后按<kbd>,</kbd>键，按照1的功能设计，会上屏一个英文‘,’号。这时只需要按一下左<kbd>Shift</kbd>键，它便可以将光标前面的英文逗号变换为中文‘，’号。在输入阿拉伯数字之后如果按<kbd>.</kbd>键，默认会上屏小数点。而如果你想输入的是中文句号的话，只需要按一下左<kbd>Shift</kbd>键来将光标前的小数点快速变换为中文‘。’号。
3. 本来在1和2功能的加持下，输入标点符号已经很爽了，已经可以和那个辣鸡中/英输入模式切换功能说bye-bye了。但可能会有老学究跳出来说：“Too young! Too simple! 中华文化博大精深，就这两把刷子，连基本的中文标点符号都弄不齐全，我要是个编字典的还不是一样累死。”想想也是……哎，我可以扩充标点按键的列表，每按一次左<kbd>Shift</kbd>键就按次序变换到下一个标点符号。这样不单止可以包括所有常用的中英文标点符号，甚至还可以安排一些常用的数学符号或者特殊符号咯。
4. 老学究还要找茬，说每按一次左<kbd>Shift</kbd>键才变换一个符号，那我想回到最前面的英文标点岂不是要按好几次才行啊。真是没完没了，好在还有个右<kbd>Shift</kbd>键。左<kbd>Shift</kbd>键按顺序轮换，而右<kbd>Shift</kbd>键则可以快速切换回英文标点。这便是上图第4列『中/英标点切换』的意思，只要按一次右<kbd>Shift</kbd>键，则在‘`→`’号左边的标点会变换成‘`→`’号右边的英文标点。 \
...
* 当光标在标点符号后面时，**按右<kbd>Shift</kbd>键可将中文标点符号或其它常用标点符号快速切换为英文标点符号**（如[设计方案图](#方案设计)中第4列『中/英标点切换』所示）。另外，当光标在成对的标点符号中间时，此功能键同样可以自动变换成对的标点符号。右<kbd>Shift</kbd>键还有一个区别于左<kbd>Shift</kbd>键的功能，就是在对`'`和`"`的变换时，只对光标前的标点进行单个变换，而不会对成对的`''`和`""`进行变换，此功能在某些场景下非常有用。
