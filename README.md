...
*温馨提示：可通过此文档右上角的〔标题列表〕按钮，快速跳转到某个标题。*
...
这个插件的目的就是要把这最后的一公里走完，让标点符号都实现中英混输，将所有中文输入法升级为真正意义上的中英混输。

## 方案设计
这个标点符号到底怎么个漂移法？在说明此插件的功能之前，得先上一张设计方案图，有了它再来忽悠就容易多了。
...
2. **下载并运行插件文件**：此项目只有1个脚本文件，`FinalD_uni.ahk`是通用版（首次运行需要做点小修改，后面会讲到），理论上可用于所有中文输入法。`FinalD_rime.ahk`是Rime输入法深度整合版，须要结合我的另一个开源项目来使用。将你想使用的文件下载到你的电脑的任意位置。
