...
| 脚本名称              | 最新版本                                                     | 备注                  |
| --------------------- | ------------------------------------------------------------ | --------------------- |
| FinalD.ahk            | [v5.63.155](https://github.com/Lantaio/IME-booster-FinalD/archive/refs/tags/v5.60.155.zip)（*2025/10/2*） | 主程序                |
| Settings\AppGroup.ahk | v1.4 (2026/3/23)                                             | 用户设置\自定义程序组 |
| Settings\Shortcut.ahk | v2.5 (2026/3/23)                                             | 用户设置\自定义快捷键 |
...
| 2025/2/8   | v5.55.133 | 增加[长按打开Rime候选窗口](#长按打开Rime候选窗口)功能和[配对标点输入改进](#配对标点输入改进) |
| 2025/10/3 | v5.60.155 | 增加[中文标点提示](#中文标点提示)功能；在有自动配对标点功能的应用程序中禁用英文标点自动配对功能；将用户自定义设置提取出来放在用户设置目录，方便日后升级；ℹ️由于此项目不再是单文件项目，安装方法有变动，详情见[安装步骤](#安装步骤%EF%B8%8F)。 |

详细更新信息见[更新日志](joy/ChangeLog.md)。

ℹ*提示：可通过此文档右上角的〔标题列表〕按钮快速转到某个标题。可通过后面有‘⚡️’图标的标题内容快速了解此项目，以及留意各部分的**粗体字**内容。<ins>有下划线的内容为近期新增或修改的部分</ins>*
...
### 长按打开Rime候选窗口
部分标点按键可以通过长按（此文档的“长按”是指超过0.2秒，下同）来打开Rime输入法标点符号候选窗口。例如我的惊喜输入方案中用<kbd>^</kbd>键触发输入扩展标点符号，用<kbd>$</kbd>键触发输入中文大写数字和金额等。
