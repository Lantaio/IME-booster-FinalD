...
* **输入标点符号时根据前一个字符是中文或是英文（包括数字）来上屏相应的标点符号，并且成对的标点符号在特定情况下会自动配对**，如上图第一列和第二列所示，主打智能和一个爽字。
* 上图第三列“英/中标点轮换”的意思是当光标在标点符号后面时，**按左<kbd>Shift</kbd>键会将光标前的标点符号作英/中轮换**。另外，当光标在成对的标点符号中间时，此功能键还可以快速变换成对的标点符号。例如：成对的`""`和`“”`互相变换,英文`[]`转换为中文`【】`、`〖〗`等。如果光标不在成对的标点中间，也可以单独替换光标前的标点符号。此功能键可理解为用于将英文标点符号变换为中文标点，后续再变换为其它常用标点的情形。当然最后也可以变换回英文标点符号。
* 上图第四列“中/英标点切换”的意思是当光标在标点符号后面时，**按右<kbd>Shift</kbd>键可将中文标点符号或其它常用标点符号快速切换为`→`右边的英文标点符号**。另外，当光标在成对的标点符号中间时，此功能键同样可以自动变换成对的标点符号。右<kbd>Shift</kbd>键还有一个区别于左<kbd>Shift</kbd>键的功能，就是在对`'`和`"`的变换时，只对光标前的标点进行单个变换，而不会对成对的`''`或`""`进行变换，在某些场景下非常有用。此功能可理解为用于快速从中文标点或其它常用标点切换为英文标点。如之后再反复按右<kbd>Shift</kbd>键，则会反复在`↔`两边的标点之间变换。所有标点都有此功能，只是为免太繁复，只用前面几个标点作为示意。
...
## 新增功能⚡️
🎊此插件从2.x版开始加入英文字母和数字的漂移功能。当光标在大、小写英文字母后面时，按左<kbd>Shift</kbd>键可以将其变换为相应的大、小写希腊字母。反之，当光标在大、小写希腊字母后面时，按右<kbd>Shift</kbd>键可以将其变换为相应的大、小写英文字母。英文字母和希腊字母的对应键位关系见下图：
...
因为英文字母和数字的漂移功能不常用，为免在输入时误按<kbd>Shift</kbd>键导致错误变换光标前的内容，因此**默认关闭**，可通过快捷键组合左<kbd>Shift</kbd>+左<kbd>Win</kbd>开/关此功能。
