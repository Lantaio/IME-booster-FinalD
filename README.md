...
📣️**公告**：v5.55版现已推出，增加长按部分标点时[呼出Rime菜单](#长按呼出Rime菜单)的功能，[配对标点输入改良](#配对标点输入改良)。详细内容见[更新日志](joy/ChangeLog.md)。

FinalD.ahk最新版本：v5.55.130（*2025/2/4*）
...
## 项目缘起⚡️
如果你是位程序员，那么很可能和我有同样的遭遇，就是用着一个号称中英混输的输入法，却还是经常须要切换中/英输入模式。为啥？原来是为了输入标点符号。而且经常因为忘记切换，导致按对按键却输错标点。或者想输入中文却发现直接上屏了英文，而要狂按<kbd>Backspace</kbd>键来改错，非常无语。
...
* 使用此输入法插件可以让大多数会自动配对和不会自动配对标点符号的编辑软件都有**一致的自动配对标点符号输入体验**。具体见下面[配对标点输入改良](#配对标点输入改良)。
...
### 大小写漂移
你是否经常遇到这种情况，就是你按了<kbd>CapsLock</kbd>键切换到大写状态输入了一些大写英文，然后忘记了处于大写状态，在想输入拼音或者五笔的时候发现上屏了几个大写英文字母。以前我只能狂按<kbd>Backspace</kbd>键，然后按一下<kbd>CapsLock</kbd>键切换回小写状态，再输一遍刚按过的几个字母按键。手速越快越崩溃。从v5.50版开始，此程序加入了几个大小写变换功能，目的是码字时不用在意<kbd>CapsLock</kbd>的状态（下表的示例中光标在最后）：
...
补充说明：光标前的英文大小写形式并不重要，漂移功能只与大小写状态有关。例如：光标前是“Test”，大小写状态处于关闭状态时，按 左<kbd>Shift</kbd>+<kbd>CapsLock</kbd>照样会将光标前的“Test”变换为“TEST”。

助记：左<kbd>Shift</kbd>+<kbd>CapsLock</kbd>是变换为全大写或者全小写，而 右<kbd>Shift</kbd>+<kbd>CapsLock</kbd>是更特别的变换功能。

### 配对标点输入改良
* 在不是自动配对的情况下，输入后标点时光标会自动回到配对标点中间。因此无论在任何地方，如果打算输入成对的标点并且在其中输入内容的话，建议你先输入成对的标点，因为这样可以避免由于前后输入的内容的中英属性不同，而导致前后配对标点不同了。
* 在非自动的配对时，短按后标点键光标会回到配对标点中间，长按（超过0.2秒）光标*不会*回到配对标点中间。
* 长按<kbd>;</kbd>键光标向右移动1格。例如有些函数没有参数，但在输入‘(’时会自动配对输入‘)’，并且光标会回到配对标点中间。这时按<kbd>;</kbd>键超过0.2秒相当于按1次<kbd>→</kbd>键。<kbd>Alt</kbd>+<kbd>Del</kbd>剪切咣标后1个字符，用于过度自动配对标点的后续操作。相应的，<kbd>Alt</kbd>+<kbd>Backspace</kbd>剪切咣标前1个字符。
...
### 长按呼出Rime菜单
部分标点按键可以通过长按（超过0.2秒）来呼出Rime菜单。例如我的惊喜输入方案中用<kbd>^</kbd>键触发输入扩展标点符号，用<kbd>$</kbd>键触发输入中文大写数字和金额等。
...
* 如果你对自定制输入法有更高的要求，有兴趣的话可以瞧瞧我的Rime输入法项目[惊喜输入方案](https://github.com/Lantaio/Rime-schema-JoySchema)。此插件为适配惊喜输入方案加入2个快捷键，当按<kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>4</kbd>（或<kbd>6</kbd>）时会弹出候选窗口，继续输入即可触发对应的高级功能。如果影响到你的使用，屏蔽这2个热键即可。
