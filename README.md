...
其它免除费用的情形：
* 如果你在GitHub上给[此项目](https://github.com/Lantaio/IME-booster-FinalD)星标⭐，并且是排名前100万名的标星用户，则可以免费获得授予给1个操作系统的此项目及所有后续更新的永久使用权，以感谢你对此项目的认可与支持🤝
...
* 输入标点符号时由于是通过选择光标前、后字符来决定上屏哪个标点，因此可能会出现短暂闪动，Word等反应慢的程序尤为明显。暂时没有更好的方法。如果对这种闪动6感到严重不适，只能放弃使用此插件。
* 小部分需要按<kbd>Shift</kbd>键上屏的标点不能通过一直按着<kbd>Shift</kbd>键来重复发送，但你可以尝试一下，大部分常见的用作重复发送的标点符号都是可以正常使用的。
...
* 在Word和PowerPoint中，英文单、双引号会自动转换为中文单、双引号，解决方法请看[这个议题](https://github.com/Lantaio/IME-booster-FinalD/issues/4)。
