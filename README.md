...
[FinalD_un.ahk](Win/FinalD_un.ahk) 通用版：v5.48.107 (*2024/12/20*) \
[FinalD_rm.ahk](Win/FinalD_rm.ahk) Rime定制版：v5.48.105 (*2024/12/20*) \
*（运行程序后用快捷键 左<kbd>Win</kbd>+<kbd>Alt</kbd>+<kbd>i</kbd>查看版本信息。）*
...
这个输入法插件（以下简称此插件）的初衷就是要走完这最后的一公里，希望通过上下文来智能输入中/英标点符号，并且可以用<kbd>Shift</kbd>键快速变换标点符号，达到无需切换中/英输入模式的目的，==**把所有中文输入法带进真正意义上的中英混输新纪元**==。
...
⚠因为英文字母和数字的漂移功能不常用，为免在输入时误按<kbd>Shift</kbd>键导致意外变换光标前的字符，**此功能默认关闭**，可通过快捷键左<kbd>Shift</kbd>+左<kbd>Win</kbd>开/关此功能。
...
| 快捷键                              | 功能                                   |
| ----------------------------------- | -------------------------------------- |
| 左<kbd>Win</kbd>+<kbd>n</kbd> | 运行/停止 此插件程序（总开关） |
| 左<kbd>Win</kbd>+<kbd>Alt</kbd>+<kbd>i</kbd> | 显示此程序的版本信息以及各项功能的状态信息  |
...
* ⚠**在输入密码时可能会有问题，导致输入的密码不正确**！暂时无法解决，只能在输入密码之前通过快捷键左<kbd>Win</kbd>+<kbd>n</kbd>停用此插件。
...
* 如果在某些情况下你需要**临时停用此插件**的话，无须关闭退出此程序，只需按快捷键左<kbd>Win</kbd>+<kbd>n</kbd>即可。留意任务栏绿色H图标会有变化。当你想恢复使用此插件时再次按此快捷键即可。（注意：用鼠标右键点击任务栏的H图标，再选择菜单中的「Pause Script」是不正确的，应选择「Suspend Hotkeys」）
