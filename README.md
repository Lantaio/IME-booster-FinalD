...
## 摘要
最新版本：[v5.60.154](https://github.com/Lantaio/IME-booster-FinalD/archive/refs/tags/v5.60.154.zip)（*2025/9/21*）
...
| 2025/2/8   | v5.55.133 | 通过[长按调出Rime候选窗口](#长按调出Rime候选窗口)和[配对标点输入改进](#配对标点输入改进) |
| 2025/9/20   | v5.60.154 | 增加[中文标点提示](#中文标点提示)功能；在有自动配对标点功能的应用程序中禁用此程序；将用户自定义设置提取出来放在用户设置目录，方便日后升级；ℹ️由于此项目不再是单文件项目，安装方法有变动，详情见『[安装步骤](#安装步骤%EF%B8%8F)』。 |
...
### 配对标点输入改进
* 在手动输入配对标点时（非自动配对），**短按后标点键**光标会自动回到配对标点中间。因此无论在任何地方，如果打算输入成对的标点并且在其中输入内容的话，建议你先输入成对的标点，因为这样可以避免由于前后输入的内容的中英属性不同，而导致前后配对标点不同。相对的，**长按[^长按时间]后标点键**（此文档的“长按”是指超过0.2秒，下同）光标*不会*回到配对标点中间。
...
* 为了更容易区分中/英文‘()’、‘'’和‘"’，在输入这些中文标点时会有提示信息，而英文则没有提示，以示区别。
...
此功能默认开启，可以通过快捷键 右<kbd>Shift</kbd>+左<kbd>Win</kbd>开/关此功能。如果不想默认打开此功能，可将此插件源代码开头处的`global BetterCN := true`中的`true`修改为`false`即可。关闭此功能代表在所有支持的应用程序中都是一致的输入体验。
...
默认为智能模式，有需要时通过快捷键切换至（表格）兼容模式。

### 中文标点提示
从v5.59版加入此功能，在输入容易混淆的中英文标点时，在输入中文标点时有提示信息，而在输入英文标点时没有提示信息。由于此功能对性能有些影响，因此默认关闭，可通过快捷键右<kbd>Ctrl</kbd>+左<kbd>Win</kbd>开/关此功能。建议在不容易区分中英文标点的应用程序中开启此功能。
...
### 升级新版
1. **下载此项目**：下载最新版的[Zip压缩包](https://github.com/Lantaio/IME-booster-FinalD/tags)，只需解压其中的Win目录到你之前存放Win目录的地方，替换所有旧文件。\
...
3. 打开Win目录中Settings目录中在UserSettings目录中也有的文件查看其版本号，如果版本号一样则表示没有更改。\
