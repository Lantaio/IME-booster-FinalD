...
📣️**公告**：4.x版现已推出，引入“中文语境应用程序组”概念，具体见[中文语境软件优化](#中文语境软件优化%EF%B8%8F)或[更新日志](joy/ChangeLog.md)。
...
## 项目缘起⚡️
如果你是一个程序员，那么很可能和我有相同的遭遇，就是用着一个号称中英混输的输入法，却还是经常须要切换中/英文输入模式来输入标点符号。甚至因为忘记切换，导致按对按键却输错标点符号而要按<kbd>Backspace</kbd>键来改错。

这个输入法插件的初衷就是要走完这最后的一公里，希望可以通过上下文来智能输入中/英文标点符号，不用切换中/英输入模式即可输入和快速变换中/英标点符号，==**把所有中文输入法带进真正意义上的中英混输新纪元**==。
...
* 上图第三列“英/中标点变换”的意思是当光标在标点符号后面时，**按左<kbd>Shift</kbd>键会将光标前的标点符号在`↔`号两边的常用中/英文标点间来回变换**。另外，当光标在成对的标点符号中间时，此功能键还可以快速变换成对的标点符号。例如：成对的`""`和`“”`互相变换,英文`[]`变换为中文`【】`，等。如果光标不在成对的标点中间，也可以单独替换光标前的标点符号。 \
...
在此插件的源代码中搜索`GroupAdd "CNApp"`，有些预设项，可按需删除或注释掉不想以中文标点为主的程序，也可以添加你希望以中文标点为主的应用程序。采用正则表达式写法，还可以查看相关[帮助文档](https://wyagd001.github.io/v2/docs/lib/GroupAdd.htm)。

此功能默认开启，可以通过快捷键右<kbd>Shift</kbd>+左<kbd>Win</kbd>开/关此功能。如果不想使用此功能，可将此插件源代码开头处的`global OptimizeCNApp := true`修改为`false`即可。
