...
[FinalD_un.ahk](Win/FinalD_un.ahk) 通用版：v5.45.100 (*2024/12/15*) \
[FinalD_rm.ahk](Win/FinalD_rm.ahk) Rime定制版：v5.45.98 (*2024/12/15*) \
...
从v5.x版开始加入兼容模式功能，并将快捷键左<kbd>Ctrl</kbd>+左<kbd>Win</kbd>改为开/关此兼容模式。此模式关闭了智能中/英标点输入和自动配对功能，保留用<kbd>Shift</kbd>键变换标点功能，使得切换到该模式时，可在各种软件的表格中正常输入和变换标点。
...
| 快捷键                              | 功能                                   |
| ----------------------------------- | -------------------------------------- |
| 左<kbd>Ctrl</kbd>+左<kbd>Win</kbd>  | 开/关 （表格）兼容模式（即 关/开 智能中/英标点输入和自动配对功能） |
...
* ⚠**不支持CMD命令提示符**，但支持PowerShell。（此程序已自动屏蔽，使用CMD时无须停用此插件。）
* ⚠**在Excel中不能使用智能标点输入和自动配对功能**,但支持Excel的VBA编辑器。编辑空白单元格时须要用鼠标3连击此单元格，编辑有内容的单元格时须要用鼠标双击此单元格。（此程序已自动屏蔽，使用Excel时无须关闭智能标点输入和自动配对功能。）
* ⚠**在各种软件的表格中不能使用智能中/英标点输入和自动配对功能**，因问题比较复杂，暂时无法解决，只能通过快捷键左<kbd>Ctrl</kbd>+左<kbd>Win</kbd>关闭“智能标点输入和自动配对功能”。
