...
📣️**公告**：v5.55版现已推出，增加部分标点[长按调出Rime候选窗口](#长按调出Rime候选窗口)的功能，[配对标点输入改进](#配对标点输入改进)。详细内容见[更新日志](joy/ChangeLog.md)。

FinalD.ahk最新版本：v5.55.130（*2025/2/5*）
...
### 配对标点输入改进
* 在不是自动的配对时，短按后标点键光标会自动回到配对标点中间。因此无论在任何地方，如果打算输入成对的标点并且在其中输入内容的话，建议你先输入成对的标点，因为这样可以避免由于前后输入的内容的中英属性不同，而导致前后配对标点不同了。
* 在不是自动的配对时，短按后标点键光标会回到配对标点中间，长按（此文档的“长按”是指超过0.2秒，下同）光标*不会*回到配对标点中间。
* 长按<kbd>;</kbd>键光标向右移动1格。例如有些函数没有参数，但在输入‘(’时会自动配对输入‘)’，并且光标会回到配对标点中间。这时长按<kbd>;</kbd>键相当于按1次<kbd>→</kbd>键。<kbd>Alt</kbd>+<kbd>Del</kbd>剪切光标后1个字符，用于过度自动配对标点的后续操作。相应的，<kbd>Alt</kbd>+<kbd>Backspace</kbd>剪切光标前1个字符。
...
### 长按调出Rime候选窗口
部分标点按键可以通过长按（超过0.2秒）来调出Rime候选窗口。例如我的惊喜输入方案中用<kbd>^</kbd>键触发输入扩展标点符号，用<kbd>$</kbd>键触发输入中文大写数字和金额等。
