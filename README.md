...
`FinalD_rime.ahk`版本：v0.27.52 \
`FinalD_uni.ahk`版本：v0.26.51 \
*用<kbd>Shift</kbd>+左<kbd>Win</kbd>快捷键组合查看版本信息。[更新日志](joy/ChangeLog.md)*

*温馨提示：可通过此文档右上角的〔标题列表〕按钮快速转到某个标题。可以通过[项目概述](#项目概述)、[主要功能](#主要功能)、[安装步骤](#安装步骤)和[已知问题](#已知问题)快速了解此项目，并留意各部分的**粗体字**内容。*
...
3. 本来在1和2功能的加持下，输入标点符号已经很爽了，已经可以和那个忽悠人的中/英输入模式切换功能说bye-bye了。但可能会有老学究跳出来说：“Too young! Too simple! 中华文化博大精深，就这两把刷子，连基本的中文标点符号都弄不齐全，我要是个编字典的还不是一样累死。”想想也是……哎，这不还有一个右<kbd>Shift</kbd>键吗？就用它来做上图第4列『常见标点轮换』的功能键，每按1次就按次序变换到下一个标点符号。这回不单止可以包括所有常用的中文标点符号，甚至还可以安排一些常用的数学符号或者特殊符号咯。
...
* 此说明文档开头有此插件的最新版本信息。如果比你在用的版本更新，只需下载新的脚本程序文件直接覆盖旧文件即可。**注意：如果你自己修改过则不能直接覆盖，否则会丢失你的自定制代码！**如想保留你的自定制代码，可以查看[更新日志](joy/ChangeLog.md)将更改合并到你的代码中。
